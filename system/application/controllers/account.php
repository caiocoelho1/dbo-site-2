...
*/
	...
	class Account extends Controller {
		...
		function index($action = 0) {
			...
			//if(empty($_SESSION['email'])) $ide->redirect(WEBSITE."/index.php/account/setEmail");
			$data = array();
	...
	function strFirst($name) {
		...
		function generate_recovery_key() {
			...
			$ide->requireLogin();
			$this->load->model("Account_model");
			if($this->Account_model->getRecoveryKey($_SESSION['name'])) {
           alert("<b>Você não pode gerar uma outra RK.</b>");
        return false;
}  
			if($_POST) {
				$data['info'] = '';
